...
This document describes smart contracts that can be setup using Epic even
though the Epic chain does not support scripting. All these contracts rely
...
document or the Epic development team. Most of the credit should be attributed
...
This section is meant as a reminder of some crucial features of the Epic chain.
...
built for a Epic transaction by taking the above private key `x` to be the sum
...
public key `r*G`, and allows to verify non-inflation for all Epic transactions
(and signs the fees).
...
receiving party. As Epic outputs are completely obscured by Pedersen
...
Alice wants to pay Bob in epics. She starts the transaction building process:
...
Alice has epics and Bob has bitcoin. They would like to swap. We assume Bob
...
ready to send her epics to Bob if he reveals `x`.

First, Alice sends her epics to a multiparty timelock contract with a refund
...
1. As soon as Bob broadcasts the final transaction to get his new epics, Alice
