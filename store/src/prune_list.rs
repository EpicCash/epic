...
//! The Epic "Prune List" implementation.
//!
...
impl PruneList {
	...
	pub fn get_total_shift(&self) -> u64 {
		self.get_shift(self.bitmap.maximum().unwrap() as u64)
	}
	...
	pub fn get_total_leaf_shift(&self) -> u64 {
		self.get_leaf_shift(self.bitmap.maximum().unwrap() as u64)
	}
	...
	fn build_pruned_cache(&mut self) {
		...
		self.pruned_cache = Bitmap::create_with_capacity(self.bitmap.maximum().unwrap());
		for pos in 1..=self.bitmap.maximum().unwrap() {
			let path = path(pos as u64, self.bitmap.maximum().unwrap() as u64);
