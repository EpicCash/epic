...
You will need to remove the directory called /chain/data. This
directory is where the epic cash blockchain stores its
data. The following step explain how to erase this data using
the terminal (if used the epic server [[#epic_config_default][default configurations]]):
