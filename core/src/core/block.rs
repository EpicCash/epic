...
impl Block {
	...
	pub fn verify_coinbase(&self) -> Result<(), Error> {
...

		{
			...
			let out_adjust_sum =
				secp.commit_sum(map_vec!(cb_outs, |x| {info!("################## x.commitment() {:?}", x.commitment());x.commitment()}), vec![over_commit])?;

			let kerns_sum = secp.commit_sum(cb_kerns.iter().map(|x|{info!("################## x.excess {:?}", x.excess); x.excess}).collect(), vec![])?;



			info!("################## over_commit {:?}", over_commit);
			info!("################## out_adjust_sum {:?}", out_adjust_sum);
			info!("################## kerns_sum {:?}", kerns_sum);
