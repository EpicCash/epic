...
impl Chain {
	...
	fn log_heads(&self) -> Result<(), Error> {
		let log_head = |name, head: Tip| {
			debug!(
				"{}: Cuckatoo {} RandomX {} ProgPow {} @ {} [{}]",
				name,
				head.total_difficulty.to_num(PoWType::Cuckatoo),
				head.total_difficulty.to_num(PoWType::RandomX),
				head.total_difficulty.to_num(PoWType::ProgPow),
				head.height,
				head.hash(),
			);
		};
	...
	pub fn rebuild_sync_mmr(&self, head: &Tip) -> Result<(), Error> {
		...
		txhashset::header_extending(&mut sync_pmmr, &sync_head, &mut batch, |ext, batch| {
