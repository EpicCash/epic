...
mod wallet_tests {
	...
	use epic_wallet::test_framework::{self, LocalWalletClient, WalletProxy};
	...
	use epic_config::GlobalWalletConfig;
	use epic_core::global;
	use epic_core::global::ChainTypes;
	use epic_keychain::ExtKeychain;
	use epic_wallet::{LMDBBackend, WalletBackend, WalletConfig, WalletInst, WalletSeed};
	...
	pub fn config_command_wallet(
		...
		if config_file_name.exists() {
			return Err(epic_wallet::ErrorKind::ArgumentError(
	...
	fn instantiate_wallet(
		...
		passphrase: &str,
		account: &str,
	) -> Result<Arc<Mutex<WalletInst<LocalWalletClient, ExtKeychain>>>, epic_wallet::Error> {
	...
	fn execute_command(
		...
		arg_vec: Vec<&str>,
	) -> Result<String, epic_wallet::Error> {
	...
	/// command line tests
	fn command_line_test_impl(test_dir: &str) -> Result<(), epic_wallet::Error> {
		setup(test_dir);
		...
		let arg_vec = vec![
			"epic", "wallet", "-p", "password", "account", "-c", "mining",
		];
		...
		let wallet1 = instantiate_wallet(config1.clone(), client1.clone(), "password", "default")?;
		epic_wallet::controller::owner_single_use(wallet1.clone(), |api| {
		...
		let wallet2 = instantiate_wallet(config2.clone(), client2.clone(), "password", "default")?;
		epic_wallet::controller::owner_single_use(wallet2.clone(), |api| {
		...
		let arg_vec = vec!["epic", "wallet", "-p", "password", "check"];
		execute_command(&app, test_dir, "wallet1", &client1, arg_vec)?;
		...
		let arg_vec = vec![
			"epic", "wallet", "-p", "password", "-a", "mining", "cancel", "-i", "26",
		];
		execute_command(&app, test_dir, "wallet1", &client1, arg_vec)?;
		...
		let arg_vec = vec![
			"epic", "wallet", "-p", "password", "-a", "mining", "txs", "-i", "10",
		];
		execute_command(&app, test_dir, "wallet1", &client1, arg_vec)?;

		// txs and outputs (mostly spit out for a visual in test logs)
		let arg_vec = vec![
			"epic", "wallet", "-p", "password", "-a", "mining", "outputs",
		];
		execute_command(&app, test_dir, "wallet1", &client1, arg_vec)?;
