...
fn mine_empty_chain() {
	...
	mine_some_on_top(".epic", pow::mine_genesis_block().unwrap(), &keychain);
}
...
fn mine_genesis_reward_chain() {
...

	{
		...
		let tmp_chain = setup(".epic.tmp", pow::mine_genesis_block().unwrap());
	...
	mine_some_on_top(".epic.genesis", genesis, &keychain);
}
...
fn mine_forks() {
	global::set_mining_mode(ChainTypes::AutomatedTesting);
	let chain = setup(".epic2", pow::mine_genesis_block().unwrap());
...
fn mine_losing_fork() {
	global::set_mining_mode(ChainTypes::AutomatedTesting);
	let kc = ExtKeychain::from_random_seed(false).unwrap();
	let chain = setup(".epic3", pow::mine_genesis_block().unwrap());
...
fn longer_fork() {
	...
	let chain = setup(".epic4", genesis.clone());
...
fn spend_in_fork_and_compact() {
	global::set_mining_mode(ChainTypes::AutomatedTesting);
	util::init_test_logger();
	let chain = setup(".epic6", pow::mine_genesis_block().unwrap());
	let prev = chain.head_header().unwrap();
	let kc = ExtKeychain::from_random_seed(false).unwrap();
	...
	let tx1 = build::transaction(
		vec![
			build::coinbase_input(consensus::reward_at_height(fork_head.height).0, key_id2.clone()),
			build::output(consensus::reward_at_height(fork_head.height).0 - 20000, key_id30.clone()),
	...
	let tx2 = build::transaction(
		vec![
			build::input(consensus::reward_at_height(next.header.clone().height).0 - 20000, key_id30.clone()),
			build::output(consensus::reward_at_height(next.header.clone().height).0 - 40000, key_id31.clone()),
			build::with_fee(20000),
		],
		&kc,
	)
...
fn actual_diff_iter_output() {
	...
	let db_env = Arc::new(store::new_env(".epic".to_string()));
	let verifier_cache = Arc::new(RwLock::new(LruVerifierCache::new()));
